...
window.addEventListener('DOMContentLoaded', function () {
    // 共通ファイル読み込み
    include(RESOURCEROOT + "/0_header.html", "body-header")
        .then(() => {
            include(RESOURCEROOT + "/0_footer.html", "body-footer")
        })
        .then(() => {
            linkClickOperation();
            bargerMenuAnimation();
...
// HTMLのパーツファイル（共通部分）読み込み
const include = function (filePath, wrapElementId) {
    return new Promise((resolve) => {
        const wrapper = document.getElementById(wrapElementId);
        if (wrapper === null) console.error(`No Element having the id "${wrapElementId}"`);
